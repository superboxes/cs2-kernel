...
namespace memory {
	...
	bool call_kernel_function(void* kernel_function_address) {
		...
		PVOID* dxgk_routine = reinterpret_cast<PVOID*>(
			memory::get_system_module_export("\\SystemRoot\\System32\\drivers\\dxgkrnl.sys", "DxgkGetSupportedVersions")
			);

		if (!dxgk_routine) {
			// fallback
			dxgk_routine = reinterpret_cast<PVOID*>(
				memory::get_system_module_export("\\SystemRoot\\System32\\drivers\\dxgkrnl.sys", "NtOpenCompositionSurfaceSectionInfo")
				);
		...
		// save original bytes
		BYTE original_bytes[12];
		memcpy(original_bytes, dxgk_routine, sizeof(original_bytes));

		// shellcoide
		BYTE shellcode[12] = { 0 };

		// First two bytes: mov rax
		shellcode[0] = 0x48;
		shellcode[1] = 0xB8;

		// Copy function address (8 bytes)
		uintptr_t function_addr = reinterpret_cast<uintptr_t>(kernel_function_address);
		memcpy(&shellcode[2], &function_addr, sizeof(function_addr));

		// Last two bytes: jmp rax
		shellcode[10] = 0xFF;
		shellcode[11] = 0xE0;

		// Write our shellcode to the target location
		write_to_read_only_memory(dxgk_routine, shellcode, sizeof(shellcode));
